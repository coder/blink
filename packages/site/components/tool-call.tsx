...
const PureToolCall = ({
  ...
  return (
    <div className="flex flex-col items-start pb-2 text-md max-w-full w-full">
      ...
      {toolInvocation.type === "tool-speak_to_user" && (
        <SpeechToolPreview
...
const seenStreamingLengthRef = new Map<string, number>();

const SpeechToolPreview = ({
  toolInvocation,
  isStreaming,
}: {
  toolInvocation: ToolUIPart<"speak_to_user">;
  isStreaming: boolean;
}) => {
  const speech = useTextToSpeech();

  // Stream audio as text grows. Once started, continue regardless of latest message status.
  useEffect(() => {
    if (!isStreaming) {
      if (seenStreamingLengthRef.has(toolInvocation.toolCallId)) {
        seenStreamingLengthRef.delete(toolInvocation.toolCallId);
        speech.send(toolInvocation.input?.parameters?.message ?? "");
      }
      return;
    }
    seenStreamingLengthRef.set(toolInvocation.toolCallId, 0);
  }, [toolInvocation.input?.parameters?.message, isStreaming]);

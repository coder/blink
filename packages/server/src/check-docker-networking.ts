...
let cachedResult: NetworkingTestResult | null = null;
...
export async function getDockerNetworkingConfig(): Promise<NetworkingTestResult> {
  ...
  cachedResult = await checkDockerNetworking();
  return cachedResult;
}

/**
 * Clear the cached networking test result (useful for testing or if Docker config changes).
 */
export function clearDockerNetworkingCache(): void {
  cachedResult = null;
}
